...
class Schedule_Display {
    ...
    public function embed_template($template) {
        ...
        if (is_page('schedule')) {
            $embed_template = SCHEDULE_DISPLAY_PLUGIN_DIR . 'templates/embed-page.php';
    ...
    public function render_schedule($atts) {
        ...
        if ($display_mode === 'calendar') {
            // カレンダー表示
            return $this->render_calendar($events, $hide_main_heading, $calendar_start_day, $debug_info, $debug_mode);
        } else {
            // リスト表示（既存のロジック）
    ...
    private function generate_calendar_month($date, $events_by_date, $start_day, $debug_mode = false) {
        ...
        // カレンダーの終了日（次月の最初の週を含める）
        $calendar_end = clone $last_day;
        $last_weekday = (int)$last_day->format('w');
        
        if ($start_day == 1) {
            // 月曜日始まりの場合
            $last_weekday = ($last_weekday == 0) ? 6 : $last_weekday - 1;
        ...
        $calendar_end->modify("+{$remaining_days} days");
        $calendar_end->setTime(23, 59, 59); // 終了日の最後の時刻に設定
        ...
        <div class="schedule-calendar">
            <h3 class="schedule-calendar-month-title"><?php echo esc_html($date->format('Y年n月')); ?> = <?php echo esc_html(date('H:i')); ?> =</h3>
            <table class="schedule-calendar-table">
                ...
                <tbody>
                    ...
                    while ($current_calendar_date <= $calendar_end) :
                        ...
                        $is_current_month = ($current_calendar_date->format('Y-m') == $date->format('Y-m'));
                            ...
                            <?php if (!empty($day_events) && is_array($day_events) && count($day_events) > 0) : ?>
                                <div class="schedule-calendar-day-events">
                                    ...
                                    foreach ($day_events as $event_index => $event) : 
                                        ...
                                        // titleが空文字列の場合はスキップしない（表示する）
                                        $event_title = trim($event['title']);
                                        if (empty($event_title)) {
                                            // タイトルが空でも表示する（「（タイトルなし）」として）
                                            $event_title = '（タイトルなし）';
                                        }
                                        
                                        if ($event_display_count >= 3) break; // 最大3件まで表示
    ...
    private function render_calendar($events, $hide_main_heading, $calendar_start_day, $debug_info, $debug_mode) {
        // イベントを日付でインデックス化
        ...
        $days_ahead = get_option('schedule_days_ahead', 60);
        ...
        $end_month->setTime(23, 59, 59); // 終了日の最後の時刻に設定
        ...
        while ($temp_date <= $end_month) {
            ...
            if (!isset($calendars[$year_month])) {
                $calendars[$year_month] = $this->generate_calendar_month($temp_date, $events_by_date, $calendar_start_day, $debug_mode);
            }
        ...
        // デバッグ情報を追加（常に表示して確認）
        $debug_calendar_info = '<div class="schedule-debug" style="margin-top: 30px; padding: 20px; background: #f5f5f5; border: 1px solid #ddd; border-radius: 8px; font-family: monospace; font-size: 12px;">';
        $debug_calendar_info .= '<h3 style="margin-top: 0; color: #333;">🔍 カレンダー表示デバッグ情報</h3>';
        $debug_calendar_info .= '<p style="color: red; font-weight: bold;">デバッグモード: ' . ($debug_mode ? '有効 (true)' : '無効 (false)') . '</p>';
            $debug_calendar_info .= '<h4>基本情報</h4>';
            $debug_calendar_info .= '<ul>';
            $debug_calendar_info .= '<li><strong>イベント総数:</strong> ' . count($events) . '件</li>';
            $debug_calendar_info .= '<li><strong>日付別イベント数:</strong> ' . count($events_by_date) . '日</li>';
            $debug_calendar_info .= '<li><strong>表示期間:</strong> ' . $start_date->format('Y-m-d') . ' ～ ' . $end_date->format('Y-m-d') . '</li>';
            $debug_calendar_info .= '<li><strong>開始月:</strong> ' . $current_date->format('Y-m') . '</li>';
            $debug_calendar_info .= '<li><strong>終了月:</strong> ' . $end_month->format('Y-m') . '</li>';
            $debug_calendar_info .= '<li><strong>生成されたカレンダー月数:</strong> ' . count($calendars) . 'ヶ月</li>';
            $debug_calendar_info .= '<li><strong>生成された月:</strong> ' . implode(', ', array_keys($calendars)) . '</li>';
            $debug_calendar_info .= '</ul>';
            
            if (!empty($events_by_date)) {
                $debug_calendar_info .= '<h4>日付別イベント（全件）</h4>';
                $debug_calendar_info .= '<pre style="background: #fff; padding: 10px; border: 1px solid #ccc; overflow-x: auto; max-height: 400px; overflow-y: auto;">';
                foreach ($events_by_date as $date_key => $day_events) {
                    $debug_calendar_info .= $date_key . ': ' . count($day_events) . '件';
                    foreach ($day_events as $event) {
                        $event_date_info = '';
                        if (isset($event['datetime']) && $event['datetime'] instanceof DateTime) {
                            $event_date_info = $event['datetime']->format('Y-m-d H:i:s') . ' (' . $event['datetime']->getTimezone()->getName() . ')';
                        } elseif (isset($event['date'])) {
                            $event_date_info = $event['date'];
                        }
                        $debug_calendar_info .= "\n  - " . (isset($event['title']) ? $event['title'] : 'N/A') . ' (date: ' . $event_date_info . ')';
                    }
                    $debug_calendar_info .= "\n\n";
                }
                $debug_calendar_info .= '</pre>';
            } else {
                $debug_calendar_info .= '<h4 style="color: red;">⚠️ 警告: イベントが日付別にマッピングされていません</h4>';
                $debug_calendar_info .= '<pre style="background: #fff; padding: 10px; border: 1px solid #ccc;">';
                $debug_calendar_info .= 'イベントデータ（最初の3件）:\n';
                $count = 0;
                foreach ($events as $event) {
                    if ($count >= 3) break;
                    $debug_calendar_info .= print_r($event, true) . "\n\n";
                    $count++;
                }
                $debug_calendar_info .= '</pre>';
            }
            ...
            if (!empty($debug_info)) {
                $debug_calendar_info .= $debug_info;
            }
            
        $debug_calendar_info .= '</div>';
        $output .= $debug_calendar_info;
